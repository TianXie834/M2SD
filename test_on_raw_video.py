...
from model.M2SD import vit_base_patch16_224
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video", required=False, type=str, default='examples/test.mp4', help="input video")
    parser.add_argument("--out_dir", required=False, type=str, default='output', help="output")

    args = parser.parse_args()

    cfg.init_with_yaml()
    cfg.update_with_yaml("M2SD.yaml")
    cfg.freeze()

    classifier = vit_base_patch16_224(num_classes=2)

    path_to_checkpoint = "save/M2SD.pt"
    ...
    for clip in tqdm(clips_for_video, desc="testing"):
        ...
        pred = float(torch.nn.functional.softmax(output, dim=-1)[0][0])
